...
//helper function: check if the current frame is already inside frames array
int contains(int cur) {
...
//helper function: update frames array
void updateFrames(int cur, int pop) {
    for (int i = 0; i < frames_available; i++) {
        if (frames[i] == pop) {
            frames[i] = cur;
            break;
...
void algorithm_FIFO() {
    ...
    //initialize the queue to track the page
    struct Queue track;
    queue_init(&track);

    int frame_left = frames_available;
    int total_page_fault = 0;

    for (int i = 0; i < reference_string_length; i++) {
        int has = contains(reference_string[i]);
        //if there is the page in the memory already
        if (has) {
            ...
            if (frame_left) {
                frame_left--;
                total_page_fault++;
                queue_enqueue(&track, reference_string[i]);
                updateFrames(reference_string[i], -1);
                display_fault_frame(reference_string[i]);
            }
            //if there is no page in memory and page replacement required
            else {
                total_page_fault++;
                int pop = queue_peek(&track);
                queue_dequeue(&track);
                queue_enqueue(&track, reference_string[i]);
                updateFrames(reference_string[i], pop);
                display_fault_frame(reference_string[i]);
            }
        }
    }
    //print the total number of the page fault
    printf(template_total_page_fault, total_page_fault);

    return;
}

// helper function for OPT: find the victim of the page replacement
int find_victim_OPT(int cur) {
    int index[MAX_FRAMES_AVAILABLE] = {};
    for (int i = 0; i < frames_available; i++) {
        index[frames[i]] = 999;
        for (int j = cur + 1; j < reference_string_length; j++) {
...
// helper function for LRU: find the victim of the page replacement
int find_victim_LRU(int history[]) {
    int victim = frames[0];
    for (int i = 0; i < frames_available; i++) {
        if (history[frames[i]] < history[victim]) {
            victim = frames[i];
        }
    }
...
void algorithm_LRU() {
    ...
    int frame_left = frames_available;
    int history[MAX_FRAMES_AVAILABLE];
    int total_page_fault = 0;

    //initialize the history to positive infinity
    for (int i = 0; i < frames_available; i++) {
        history[i] = 999;
    }
    ...
    for (int i = 0; i < reference_string_length; i++) {
        ...
        //if there is the page in the memory already
        if (has) {
            history[reference_string[i]] = i;
            printf(template_no_page_fault, reference_string[i]);
        }
        else {
            //if there is no page in memory but there are still avilable slots
            if (frame_left) {
                frame_left--;
                total_page_fault++;
                updateFrames(reference_string[i], -1);
            ...
            //if there is no page in memory and page replacement required
                ...
                total_page_fault++;
                int pop = find_victim_LRU(history);
                updateFrames(reference_string[i], pop);
                history[reference_string[i]] = i;
                display_fault_frame(reference_string[i]);
            }
